...
class SoundController(object):
    ...
    def play(self,key, loops=0, max_time=0, fade_ms=0):
        """ """
        if not self.enabled: return
        # if beand and franks is running - don't play any other switch noises
        if key in self.sounds:
            if len(self.sounds[key]) > 0:
                random.shuffle(self.sounds[key])
            # set the list of things that can play during franks
            allowed_keys = ["sfx_franks","music_gunfightIntro",
                            "sfx_gunfightFlourish",
                            "sfx_gunfightHit1",
                            "sfx_gunfightHit2",
                            "sfx_gunfightHit3",
                            "sfx_gunCock",
                            "sfx_smashingWood",
                            "sfx_orchestraFlourish"]
            if self.franks and key not in allowed_keys:
                pass
            else:
